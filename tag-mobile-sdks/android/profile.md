...
To obtain the payment instruments from the profile that are accepted by a specified <code>Merchant</code>:
	...
	<code>Profile profile = ProfileManager.getInstance().getCurrentProfile();
	List&lt;PaymentInstrument&gt; acceptedPaymentInstruments = profile.getAcceptedPaymentInstruments(merchant);</code>
